...
void socket_pool::routine()
{
    POOL_BGN
    {
        LOG_INF("Connection pool monitor. Started");
        asio::io_context ctx;
        tcp::socket sock(ctx);
        ...
        while (enable())
        {
            ...
            tp = now + std::chrono::seconds(30);
            {
                ...
                for (; it != m_ready.end(); ++it) {
                    ...
                    if (it->socket != -1) {
                        ...
                        if (!ec) {
                            ...
                            LOG_INF("Connection pool monitor. Shutdown socket %s. Error (%u) %s", it->ep.c_str(), ec.value(), ec.message().c_str());
                            ec.clear();
                            sock.close(ec);
                            LOG_INF("Connection pool monitor. Close socket %s. Error (%u) %s", it->ep.c_str(), ec.value(), ec.message().c_str());
                        } else {
                            LOG_INF("Connection pool monitor. Assign socket %s. Error (%u) %s", it->ep.c_str(), ec.value(), ec.message().c_str());
                        }
                    }
                    del.push_back(it);
                }
                ...
                LOG_INF("Connection pool monitor. Ready/Busy: %u/%u", m_ready.size(), m_busy.size());
            }
    ...
    POOL_END()
    cleanup();
    LOG_INF("Connection pool monitor. Stoped");
}
