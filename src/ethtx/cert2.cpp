...
std::string DeriveAESKeyFromPasswordDefault(const std::string& password, std::string& newsalt, bool isLight)
{
    std::string aeskey = "";
    ...
    uint64_t N = isLight ? SCRYPT_LIGHT_N : SCRYPT_DEFAULT_N;
...
std::pair<std::string, std::string> EncodePrivKey(const std::string& privkey, const std::string& password, bool isLight)
{
    ...
    {
        std::string derivedkey = DeriveAESKeyFromPasswordDefault(password, newsalt, isLight);
        if (!derivedkey.empty())
        {
            ...
            certparams.n = isLight ? SCRYPT_LIGHT_N : SCRYPT_DEFAULT_N;
...
std::pair<std::string, std::string> CreateNewKey(const std::string& password, bool isLight) {
    std::string rawprivkey = CreateRawECDSAKey();
    CHECK(!rawprivkey.empty(), "rawprivkey empty");
    return EncodePrivKey(rawprivkey, password, isLight);
}
