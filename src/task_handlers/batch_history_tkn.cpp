...
batch_history_tkn::batch_history_tkn(http_session_ptr session)
    : base_network_handler(settings::server::address, session) {
    m_duration.set_message(__FUNCTION__);
}

batch_history_tkn::~batch_history_tkn() {
}

bool batch_history_tkn::prepare_params()
{
    BGN_TRY
    {
        ...
        m_writer.add("method", "tkn.batch.last.history");
