...
// fetch_balance_tkn
fetch_balance_tkn::fetch_balance_tkn(http_session_ptr session):
    base_network_handler(settings::server::address, session),
    m_exec(!settings::service::local_data) {
    m_duration.set_message(__func__);
}

fetch_balance_tkn::~fetch_balance_tkn() {
}

bool fetch_balance_tkn::prepare_params()
{
    BGN_TRY
    {
        ...
        if (!m_exec) {
            ...
            std::string db_res;
            CHK_PRM(storage::database::get_tkn_balance(addr, db_res), "failed on get token balance from database")
        ...
        */
        m_writer.add("method", "address.tkn.balance");
