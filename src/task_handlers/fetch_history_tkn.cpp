...
fetch_history_tkn::~fetch_history_tkn() {
}

bool fetch_history_tkn::prepare_params()
{
    BGN_TRY
    {
        ...
        if (!m_exec) {
            ...
            std::string db_res;
            storage::database::get_tkn_history(addr, db_res);
            if (db_res.empty()) {
                db_res = "[]";
            }
            ...
            CHK_PRM(reader.get_doc().IsArray(), "history token from db has incorrect type");
        ...
        */
        m_writer.add("method", "address.tkn.transaction");
...
void fetch_history_tkn::execute() {
    if (m_exec) {
